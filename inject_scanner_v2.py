...
BOT = Path("bot.py")
SCANNER = Path("enhanced_scanner.py")

bot_text = BOT.read_text()
scanner_backup = Path(f"enhanced_scanner_backup_v2_{datetime.now().strftime('%Y%m%d_%H%M%S')}.py")
bot_backup = Path(f"bot_backup_scanner_v2_{datetime.now().strftime('%Y%m%d_%H%M%S')}.py")

scanner_backup.write_text(SCANNER.read_text() if SCANNER.exists() else "")
...
old = '''    dynamic_watchlist = list(dict.fromkeys(WATCHLIST + small_caps))
    log(f"Dynamic watchlist: {dynamic_watchlist}")
'''

new = '''    dynamic_watchlist = list(dict.fromkeys(WATCHLIST + small_caps + enhanced))
    log(f"Dynamic watchlist: {dynamic_watchlist}")
    log(f"SCANNER | enhanced candidates={enhanced}")
'''

if old not in bot_text:
    raise SystemExit("Could not find dynamic_watchlist block. bot.py not changed.")

bot_text = bot_text.replace(old, new)

BOT.write_text(bot_text)

print("✅ Scanner v2 installed")
print(f"✅ Bot backup: {bot_backup}")
print(f"✅ Scanner backup: {scanner_backup}")
print("Now run:")
print("python3 -m py_compile bot.py enhanced_scanner.py")
