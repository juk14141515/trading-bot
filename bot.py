...
def finnhub_analyst_score(symbol):
    try:
        ...
        score = (strong_buy * 2) + buy_count - sell_count - (strong_sell * 2)
...
def news_score(symbol):
    try:
        ...
        for article in news:
            headline = safe_headline_text(article).lower()
...
def buy(symbol, score, entry_reason=None, setup="current_bot"):
    ...
    if shares <= 0:
        ...
        set_why_not_trading(
            f"Best candidate {symbol} blocked by position sizing.",
    ...
    if deployed + dollars > max_deployed:
        ...
        set_why_not_trading(
            f"Best candidate {symbol} would exceed max deployed capital.",
            "Would exceed max deployed",
            "blocked"
        )
        return
...
def manage_existing_positions():
    ...
    for p in positions:
        symbol = p.symbol
        # TRADE GUARD V1: conservative fast loser protection
        try:
            qty = float(p.qty)
...
def run_bot():
    ...
    if not market_is_open():
        ...
        update_status({
            ...
            "slots_available": 0,
            "why_not_trading": "Market is currently closed.",
            "last_skip_reason": "Market closed",
    ...
    set_why_not_trading("Scanning for qualified trades.", "", "scanning")

    if TRADING_HALTED:
        log("SKIP BUY | trading halted for today")
        set_why_not_trading(
            "Trading is halted for today.",
            "Max daily loss halt or manual halt",
            "halted"
        )
        return
    ...
    if market_trend != "bullish":
        ...
        set_why_not_trading(
            f"SPY trend is {market_trend}; bot only buys in bullish markets.",
            f"SPY not bullish ({market_trend})",
            "waiting"
        )
        return
    ...
    dynamic_watchlist = list(dict.fromkeys(WATCHLIST + small_caps + enhanced))
    log(f"Dynamic watchlist: {dynamic_watchlist}")
    log(f"SCANNER | enhanced candidates={enhanced}")
    if enhanced:
    ...
    if len(candidates) == 0:
        set_why_not_trading(
            "No qualified candidates found this cycle.",
            "Scanner found no bullish candidates that passed score inputs",
            "waiting"
        )
        return
    ...
    for candidate in candidates[:slots_available]:
        ...
        if not allowed:
            ...
            set_why_not_trading(
                f"Best candidate {symbol} blocked by risk manager.",
                reason,
                "blocked"
            )
            continue
