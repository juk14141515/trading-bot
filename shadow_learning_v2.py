...
# Research-only thresholds. These do not place orders.
...
def log_small_cap_shadow(candidates):
    ...
    for raw in candidates or []:
        ...
        if cand["score"] < SMALL_CAP_THRESHOLD:
            continue
        decision = "research_small_cap_candidate"
        reason = "Research signal: candidate fits small-capital learning threshold."
        log_learning_event(
            ...
            reason=reason,
            rotation_decision=decision,
            notes=f"shadow_only=true | version={SHADOW_VERSION} | sim_capital={SMALL_CAP_SIM_CAPITAL} | max_position={SMALL_CAP_MAX_POSITION} | max_positions={SMALL_CAP_MAX_POSITIONS} | trend={cand['trend']} | analyst={cand['analyst']} | news={cand['news']}",
        )
...
def log_day_trade_shadow(candidates):
    ...
    for raw in candidates or []:
        cand = candidate_dict(raw)
        if cand["score"] < DAY_TRADE_THRESHOLD:
            continue
        if cand["news"] >= 1:
            decision = "research_day_trade_confirmed"
            reason = "Research signal: aggressive day-trade candidate with catalyst confirmation."
        else:
            decision = "research_day_trade_unconfirmed"
            reason = "Research signal: aggressive day-trade candidate without catalyst confirmation."
        log_learning_event(
            ...
            reason=reason,
            rotation_decision=decision,
            notes=f"shadow_only=true | version={SHADOW_VERSION} | aggressive_mode=true | sim_capital={SMALL_CAP_SIM_CAPITAL} | trend={cand['trend']} | analyst={cand['analyst']} | news={cand['news']}",
        )
        events.append({"symbol": cand["symbol"], "decision": decision, "score": cand["score"]})
    return events
...
def log_fast_signal_candidates(candidates):
    ...
    for raw in candidates or []:
        ...
        log_learning_event(
            ...
            reason=reason,
            rotation_decision=decision,
            notes=f"shadow_only=true | version={SHADOW_VERSION} | sim_capital={SMALL_CAP_SIM_CAPITAL} | trend={cand['trend']} | analyst={cand['analyst']} | news={cand['news']}",
        )
        events.append({"symbol": cand["symbol"], "decision": decision, "score": cand["score"]})
    return events
...
def run_shadow_learning_v2(positions=None, candidates=None, log_func=None):
    try:
        ...
        small_cap_events = log_small_cap_shadow(candidates or [])
        day_trade_events = log_day_trade_shadow(candidates or [])
        fast_events = log_fast_signal_candidates(candidates or [])
        if log_func:
            log_func(
                f"SHADOW V2 | sell={len(sell_events)} | capital={'yes' if efficiency else 'no'} | small_cap={len(small_cap_events)} | day_trade={len(day_trade_events)} | fast={len(fast_events)}"
            )
        return {
            "mode": "shadow_only",
            ...
            "small_cap_events": len(small_cap_events),
            "day_trade_events": len(day_trade_events),
