...
backup = Path(f"bot_backup_adaptive_learning_{datetime.now().strftime('%Y%m%d_%H%M%S')}.py")
text = BOT.read_text()
backup.write_text(text)

ADAPTIVE.write_text(r'''
import csv
import os
...
# imports
if "from adaptive_learning import apply_adaptive_score, get_performance_summary" not in text:
    text = text.replace(
        "from risk_manager import can_trade",
        "from risk_manager import can_trade\nfrom adaptive_learning import apply_adaptive_score, get_performance_summary"
    )

if "from trade_memory import record_trade" not in text:
    text = text.replace(
        "from risk_manager import can_trade",
        "from risk_manager import can_trade\nfrom trade_memory import record_trade"
    )

# patch candidate scoring
old = '''    for symbol, score in candidates[:slots_available]:
        final_score = score
'''

new = '''    for symbol, score in candidates[:slots_available]:
...
if old in text:
    ...
    print("WARNING: candidate score block not found")

# patch buy record
old_buy = '''    notify_discord(f"🟢 BUY PLACED | {symbol} | ${dollars:.2f} | score={score}")
'''

new_buy = '''    notify_discord(f"🟢 BUY PLACED | {symbol} | ${dollars:.2f} | score={score}")
...
if old_buy in text:
    ...
    print("WARNING: buy record patch point not found")

# patch sell function to record pnl
old_sell = '''def sell(symbol, qty, reason):
    api.submit_order(
        symbol=symbol,
        qty=qty,
        side="sell",
    ...
    log(f"SELL | {symbol} | reason={reason}")
    notify_discord(f"🔴 SELL PLACED | {symbol} | qty={qty} | reason={reason}")
'''

new_sell = '''def sell(symbol, qty, reason):
    sell_price = None
    pnl = None
    pnl_pct = None
...
if old_sell in text:
    ...
    print("WARNING: sell function patch point not found")

BOT.write_text(text)

print("✅ Adaptive learning system installed")
print(f"✅ Backup created: {backup}")
print("Now run:")
print("python3 -m py_compile bot.py adaptive_learning.py trade_memory.py")
