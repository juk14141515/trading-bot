...
def summarize_learning(events: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    small_cap = [e for e in events if e["event"] == "SHADOW_SMALL_CAP_BUY_DECISION" or e["score"] >= SMALL_CAP_THRESHOLD and "small" in e.get("notes", "").lower()]
    day_trades = [e for e in events if e["event"] in {"SHADOW_DAY_TRADE_SIGNAL", "SHADOW_FAST_SIGNAL"} or e["score"] >= DAY_TRADE_THRESHOLD and "fast" in e["event"]]
    ...
    return {
        ...
        "paper_70_plus_decisions": len(paper_decisions),
...
def simulate_small_cap(events: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    # This is deliberately a decision simulator, not a price/PnL simulator yet.
    # It answers: would this setup be eligible for a $500-$1k account model?
...
def build_outputs() -> Dict[str, Any]:
    ...
    backtest = {
        ...
        "learning_summary": learning_summary,
        "note": "This is not the old SMA/RSI backtest. It summarizes bot output data until enough outcomes exist for true score-based backtesting.",
    }
    ...
    return {
        "forward": forward,
        "backtest": backtest,
        "setup_performance": setup_output,
    }
